...
use aws_sdk_controltower::operation::list_enabled_controls::{
    ListEnabledControlsError, ListEnabledControlsOutput,
};
...
impl ControlTowerImpl {
    ...
    pub async fn list_enabled_controls(
        &self,
    ) -> Result<ListEnabledControlsOutput, SdkError<ListEnabledControlsError>> {
...
mod test {
    ...
    use aws_sdk_controltower::types::{
        DriftStatusSummary, EnabledControlSummary, EnablementStatusSummary,
    };
    ...
    async fn test_list_landing_zones() {
        ...
        mock.expect_list_landing_zones().return_once(|| {
            Ok(ListLandingZonesOutput::builder()
                .landing_zones(
                    LandingZoneSummary::builder()
                        .arn("arn:aws:controltower:example:landingzone/1")
                        .build(),
                )
                .build()
                .expect("Failed to build ListLandingZonesOutput"))
        });
        ...
        assert_eq!(
            landing_zone_arns,
            vec!["arn:aws:controltower:example:landingzone/1".to_string()]
        );
    }
    #[tokio::test]
    async fn test_list_enabled_controls() {
        ...
        mock.expect_list_enabled_controls().return_once(|| {
            Ok(ListEnabledControlsOutput::builder()
                    ...
                    EnabledControlSummary::builder()
                        ...
                        .status_summary(
                            EnablementStatusSummary::builder()
                                .status("SUCCEEDED".to_string().parse().unwrap())
                                .build(),
                        )
                        .drift_status_summary(
                            DriftStatusSummary::builder()
                                .drift_status("NOT_CHECKING".to_string().parse().unwrap())
                                .build(),
                        )
                        .build(),
                )
                ...
                .build()
                .expect("Failed to build ListEnabledControlsOutput"))
        });
        ...
        let enabled_control_drift_statuses: Vec<String> = enabled_controls_output
            ...
            .collect();
        assert_eq!(
            enabled_control_drift_statuses,
            vec!["NOT_CHECKING".to_string(), "NOT_CHECKING".to_string()]
        );
    }
}
